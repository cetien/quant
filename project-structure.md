...
│
├── storage/                       # 저장 레이어
│   ├── db_manager.py              # DuckDB 연결, CRUD, upsert(append-only)
...
│   ├── scorer.py                  # Z-score 정규화 + 가중합 최종 점수
...
| **storage** | `schema.py` | DDL 7개 테이블 (announce_date·delisted_stocks·CHECK 제약) |
| | `db_manager.py` | append-only upsert, as_of_date 조회, Zero-Copy |
| | `parquet_store.py` | 연/월 파티션 저장·DuckDB read_parquet 로드 |
| **ingestion** | `price_collector.py` | pykrx 일봉+거래대금, 증분 업데이트 |
| | `supply_collector.py` | 기관·외인 순매수 (pykrx) |
...
| | `settings.py` | 수집 트리거 (Phase 0~1) |
| **루트** | `main.py` | Phase별 주석 해제 실행 |
| | `requirements.txt` | 전체 의존성 |
| | `README.md` | 실행 순서+설계 원칙 |
